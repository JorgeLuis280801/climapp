...
class WeatherAPI {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('Muriooo: ${response.statusCode}');
    }
  } catch (e) {
    print('Error: $e');
  }
...
class WeatherAPI {
  ...
  Future<List<ListElement>?> getCurrent() async {
  try {
    Position position = await obt_Posicion();
  ...
  Future<List<ListElement>?> getTemp(
      {required double lat, required double lon}) async {
    try {
      link = Uri.parse(
          'https://api.openweathermap.org/data/2.5/forecast?cnt=8&units=metric&lat=${lat}&lon=${lon}&appid=5108ff74cd677c638147f2c6f053e7ae');
